...
describe('PIN DEK wrapping (property-based)', () => {
  it(
    'round-trips for any valid PIN and random DEK',
    { timeout: 120_000 },
    async () => {
      await fc.assert(
        fc.asyncProperty(validPinArb, fc.uint8Array({ minLength: 32, maxLength: 32 }), async (pin, dek) => {
  ...
  it(
    'wrong PIN never returns the original DEK',
    { timeout: 120_000 },
    async () => {
      await fc.assert(
        fc.asyncProperty(
